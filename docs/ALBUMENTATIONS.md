...
## 🔍 Visualize augmentation script

There is a helper script `scripts/visualize_augmentations.py` that shows a tile and several sampled augmentations side-by-side. It's useful for verifying geometric transform alignment and visualizing photometric effects.

Usage example:

```bash
python scripts/visualize_augmentations.py --tile 00000 --data_root data/tiles --n 5 --seed 42
```

Key features:
- Supports multispectral and RGB tiles
- Deterministic augmentation sampling with `--seed`
- Choose RGB display channels via `--channels 2,1,0`
- Save the figure with `--out out.png` or suppress display with `--no_show`
